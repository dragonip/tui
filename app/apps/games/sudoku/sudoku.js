...
function showHint() {
	...
	for (var row = 0; row < 9; row++) {
		for (var col = 0; col < 9; col++) {
			...
			if (!isCorrect(obj)) {
				...
				hintDiv.style.top = getTopPos(obj) + 'px';
	...
	if (objectToTry) {
		...
		hintDiv.style.top = getTopPos(objectToTry) + 'px';
		hintDiv.style.display = 'block';
	}
}
...
/*if (e.keyCode) code = e.keyCode;
	...
	var remotecodes = ['one','two','three','four','five','six','seven','eight','nine'];
	var ll = remotecodes.indexOf(code);
	if (ll != -1 ) {
		ll = ll+1;
		span.innerHTML = ll;
	}
...
function findFirstEmpty() {
	var a = document.getElementById('sudokuSection0');
	var b = a.querySelectorAll('.sudokuSquare');
	var i, el;
	for (i = 0; i < b.length; i++) {
		if (!b[i].style.backgroundColor) {
			//found enpty cell
			el = b[i];
			break;
		}
