...
//
/**
 * @module debug/eyes Provides HTML rendering of JSON objects
 * @require shims/foreach shims/objectkeys shims/indexof
 * */
define(['shims/foreach','shims/objectkeys', 'shims/indexof', 'shims/some'], function() {
	var eyes = {};
	var stack = [];
	eyes.defaults = {
		...
		hideFunctions: false,
		stream: undefined,
		maxLength: 102048 // Truncate output if longer
	};
...
/*return str.replace(/\\/g, '\\\\')
	...
	function stringifyArray(ary, options, level) {
		...
		var pretty = options.pretty && (ary.length > 4 || ary.some(function(o) {
			return (typeof(o) === 'object' && Object.keys(o).length > 0) || (Array.isArray(o) && o.length > 0);
		}));
		var ws = pretty ? '<br>' + new(Array)((level * 4)+1).join('&nbsp;') : '&nbsp;';
		var bws = pretty ? '<br>' + new(Array)(((level -1 )* 4)+1).join('&nbsp;') : '&nbsp;';
		...
		if (out.length === 0) {
			...
			return '[' + ws + out.join(',' +  (pretty ? ws : ' ')) + bws + ']';
		}
	};
	...
	function stringifyObject(obj, options, level) {
		...
		var ws = pretty ? '<br>' + new(Array)(level * 4 + 1).join('&nbsp;') : '&nbsp;';
		var bws = pretty ? '<br>' + new(Array)((level -1 )* 4).join('&nbsp;') : '&nbsp;';
	...
	var type_names = ['Object','Array','String','RegExp', 'Number', 'Function', 'Boolean', 'Date'];
	function typeOf(value) {
		var s = typeof(value),
			types = [Object, Array, String, RegExp, Number, Function, Boolean, Date];
		if (s === 'object' || s === 'function') {
			if (value) {
				types.forEach(function(t,ii) {
					if (value instanceof t) {
						//Use this work around, IE7 does not have .name property on objects
						s = type_names[ii].toLowerCase();
						//
	...
	/**
	 * @method inspect Renders JSON object nesting to HTML print
	 * @param {JSON Object} obj. Any JS object worth printing with indentation
	 * @return {String} HTML representation of the JSON object
	 * */
	return eyes;
});
