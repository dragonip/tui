...
define(function() {
	...
	var events = {},
		...
		onReadyBound = false,
		isReady = false,
		DOMContentLoaded, readyCallbacks = [],
		Emitter;
	...
	function fix(event, element) {
		...
		if (typeof event.target === 'undefined') event.target = event.srcElement || element;
	...
	events.addDOMethods = function() {
		...
		var chainedAliases = ('click dblclick mouseover mouseout mousemove ' + 'mousedowe mouseup blur focus change keydown ' + 'keypress keyup resize scroll').split(' ');
